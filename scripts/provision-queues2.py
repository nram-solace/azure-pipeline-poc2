...
def main(argv):
    ...
    p.add_argument('--inventory', '-i', dest="inventory_file", required=True, help='Ansible style inventory file') 
    p.add_argument('--input-file','-f', dest="input_file", required=True, help='input data file') 


    r = p.parse_args()

    # read inventory
    inv = read_config_ini(r.inventory_file)
    pp.pprint(inv)
...
def provision_queues(sempurl, user, passwd, queues, template_data):

    vpn = queues['defaults']['msgVpnName']
    url = f'{sempurl}/{config_url}/msgVpns/{vpn}/queues'
    ...
    for queue in queues['queues']:
        ...
        if status == 200:
            ...
            update_queue(q_url, user, passwd, temp_data)

            # send update properties now
            print (f'Updating queue {queue}')
            update_queue(q_url, user, passwd, data)
        else:
            print (f'Queue does not exist. Create queue {queue}')
            create_queue(url, user, passwd, data)

def read_config_ini (file, config={}):
    cfg = config.copy()
    cp = ConfigParser()
    cp.read(file)
    for sec in cp.sections():
        ...
        for opt in cp.options(sec):
            cfg[name + "." + opt.lower()] = cp.get(sec, opt).strip()
    return cfg
